...
var Animate = (function() {
	...
	_Animate.prototype.bind = function() {
		...
		$(document).on('click', '.add_li', function(e) {
			...
			.on('click', '.deleted_node', function(e) {
				var Index = $('.deleted_node').index(this)

				$('.content_inside_li').eq(Index).fadeOut()
			})
			.on('mouseover', '.items_children_li', function(e) {
				$(e.target).find('.deteled_child').css('display', 'block')
				setTimeout(function() {
					$(e.target).find('.deteled_child').fadeOut()
				}, 5000)
			})
			.on('click', '.deteled_child', function(e) {
				if(e.target.nodeName == 'I') {
					$(e.target).parent().parent().animate({opacity:0},500,function(){
						$(e.target).parent().parent().remove()
					})
				} else {
					$(e.target).parent().animate({opacity:0},500,function(){
